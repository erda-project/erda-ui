...
export const Avatar = (props: IProps) => {
  ...
  if (userAvatar) {
    return wrapName(<img className={cls} style={style} src={ossImg(userAvatar, { w: Math.floor(size * 1.2) })} alt="user-avatar" />);
  }
...
export const AvatarList = ({
  ...
  return (
    <Tooltip
      title={tooltipElements}
      overlayStyle={{ maxWidth: 200 }}
    >
      {
        names.length > maxDisplay
          ? [...displayElements.slice(0, maxDisplay), <span key="has-more" className="dice-avatar-has-more">...</span>]
          : displayElements
      }
    </Tooltip>
  );

...
import { Form as NForm } from 'nusi';
    ...
    <NForm {...formProps}>
      {fields.map((f: FormField, index:number) => {
        ...
        if (group) {
            ...
            return (
              <Component key={f.key || `${index}`} fieldConfig={f} {...rest}>
                {map(filter(groupFields[group], i => i.component !== FORM_GROUP), (gItem, subIndex) => {
                  ...
                  return SubComponent ? <SubComponent key={gItem.key || `${index}-${subIndex}`} fieldConfig={gItem} form={form} getLabel={getLabel} {...rest} /> : null;
                })}
              </Component>
            );
          }
          ...
          return Component ? <Component key={f.key || `${index}`} fieldConfig={f} form={form} getLabel={getLabel} {...rest} /> : null;
        }
      })}
    </NForm>
  );

...
export const defaultPreviewField = ({
  ...
  currentEditField,
  onCopy,
}: any) => (compMap: any) => ({ fields, form }: any) => {
  return (
    <div>
      ...
      map(curConfig, (item) => {
        ...
        map(item, (objItem, key) => {
          if (typeof objItem !== 'function') {
            let useable = !((key === 'validateTrigger' && objItem.toString() === 'onChange') || ((isPlainObject(objItem) || isArray(objItem) || isString(objItem)) && isEmpty(objItem)));
        ...
        const { valid, isTouched, converted, remove, visible, _disabledSubscribes, _disabledWatchers, _hideSubscribes, _hideWatchers, _removeSubscribes, _removeWatchers, ...rest } = configItem as any;
  ...
  return (
    <div className="dice-form-preview">
      ...
      <Modal
        ...
        visible={modalVis}
        onOk={onOk}
        onCancel={() => { setModalVis(false); setCopyData({}); }}
      >
        <Form
          fields={addFields}
          formRef={addForm}
      ...
      >
        { groupModalVis ? (
          <Form
            key={get(curGroup, 'group') || '_'}
            fields={addGroupItemFields}
            formRef={addGroupForm}
          />
        ) : null}
      </Modal>

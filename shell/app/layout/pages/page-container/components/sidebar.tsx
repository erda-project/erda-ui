				...
				<Tooltip
					title={
						currentOrg?.id
							? ""
							: i18n.t(
									"layout:there is no organization information, please select an organization first"
							  )
					}
					placement="right"
				>
		...
		},
		{
			...
			icon: (
				<Badge
					...
					count={unreadCount}
					offset={[-5, 2]}
					style={{ width: "4px", height: "4px", boxShadow: "none" }}
				>
...
const PopoverSelector = (props: IPopoverSelectorProps) => {
	...
	const ValueRender = (
		<div
			className=" v-align side-app-center flex-box pointer"
	...
	return (
		<Popover
			content={menu}
			placement="bottom"
			overlayClassName="side-app-center-popover"
		>
			{ValueRender}
		</Popover>
	);

...
import { ErdaAlert, Icon as CustomIcon } from 'common';
    ...
    <>
      ...
      {cluster && (
        <>
          <Row>
            <Col span={12}>
              <Form.Item
                ...
                className="my-4 pr-1"
                rules={[
                  { required: true, message: i18n.t('Please enter the {name}', { name: 'CPU' }) },
                  {
            ...
            <Col span={12}>
              <Form.Item
                ...
                className="my-4 pl-1"
                rules={[
                  { required: true, message: i18n.t('Please enter the {name}', { name: i18n.t('memory') }) },
                  {
                    validator: async (_rule: any, value: any) => {
                      if (value && (isNaN(+value) || +value < 0 || `${value}`.split('.')[1]?.length > 3)) {
                        throw new Error(i18n.t('Please enter the number of {min}-{max} decimal', { min: 1, max: 3 }));
          ...
          {showTip && <ErdaAlert message={tip} type="info" className="mb-4" />}
        </>
      )}
    </>
  );

...
const BlockNetworkStatus = ({ status, canOperate = false, onClick, scope, unBlockStart, unBlockEnd }: IProps) => {
  ...
  const statusMap: { [key in PROJECT.BlockStatus]: React.ReactNode } = {
    ...
    unblocking: (
      <>
        <span className="inline-flex justify-between items-center text-warning">
          <CustomIcon type="lock1" className="text-warning" />
    ...
    unblocked: (
      <>
        <span className="inline-flex justify-between items-center text-success">
          <CustomIcon type="unlock1" className="text-success" />

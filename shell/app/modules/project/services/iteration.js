...
export function getIterations(payload) {
  ...
  return agent
    .get(`/api/project/${projectId}/iteration`)
    .query(extra)
...
export function getIssues({ iterationId, ...rest }) {
  return agent
    .get(`/api/iteration/${iterationId}/issue`)
    .query(rest)
    .then((response) => response.body);
}
// 发布迭代
export function iterationRelease({ iterationId, releaseLog, version }) {
  return agent
    .put(`/api/iteration/${iterationId}/release`)
...
export function getFreeIssues({ projectId, ...rest }) {
  return agent
    .post(`/api/project/${projectId}/unreleased-issue`)
    .send(rest)
    .then((response) => response.body);
}
...
export function createIteration(iterationInfo, projectId) {
  return agent
    .post(`/api/project/${projectId}/iteration`)
    .send(iterationInfo)
    .then((response) => response.body);
}
...
export function updateIteration(iterationId, iterationInfo) {
  return agent
    .put(`/api/iteration/${iterationId}`)
    .send(iterationInfo)
    .then((response) => response.body);
}

export function delIteration(iterationId) {
  return agent.delete(`/api/iteration/${iterationId}`).then((response) => response.body);
}

export function addIssuesToIteration(iterationId, issueArr) {
  return agent
    .put(`/api/iteration/${iterationId}/issue`)
    .send(issueArr)
    .then((response) => response.body);
}
...
export function topIteration(iterationId, toTop) {
  return agent
    .post('/api/iteration/top')
    .send({ toTop, id: iterationId })
    .then((response) => response.body);
}
export function getIterationStatus() {
  return agent.get('/api/iteration/status').then((response) => response.body);
}

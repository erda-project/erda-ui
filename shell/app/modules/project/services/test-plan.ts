...
export function getTestPlanItem(testPlanID: number): TEST_PLAN.PlanBodySimpleResponse {
  return agent.get(`/api/testplans/${testPlanID}`)
    .then((response: any) => response.body);
}
...
export function addToPlan({ testPlanID, ...rest }: TEST_PLAN.relationsCase): {totalCount: number} {
  return agent.post(`/api/testplans/${testPlanID}/testcase-relations`)
    .send(rest)
    .then((response: any) => response.body);
}

export function getReport(testPlanID: number): TEST_PLAN.PlanReportResponse {
  return agent.get(`/api/testplans/${testPlanID}/actions/generate-report`)
    .then((response: any) => response.body);
}

export function updateTestCaseRelations({ testPlanID, ...payload }: TEST_PLAN.PlanBatch) {
  return agent.post(`/api/testplans/${testPlanID}/testcase-relations/actions/batch-update`)
    .send(payload)
    .then((response: any) => response.body);
}
...
export function executeCaseApi({ testPlanID, ...rest }: TEST_PLAN.CaseApi) {
  return agent.post(`/api/testplans/${testPlanID}/actions/execute-apitest`)
    .send({ ...rest })
    .then((response: any) => response.body);
}
...
export const getExecuteRecordDetail = ({ runVersion, pipelineID }: TEST_PLAN.ExecuteRecordDetailQuery): TEST_PLAN.PipeLineDetail => {
  return agent.get(`/api/apitests/pipeline/${pipelineID}`)
    .query({ rerunVer: runVersion })
    .then((response: any) => response.body);
};

export const cancelBuild = ({ testPlanID, pipelineID }: {testPlanID: string; pipelineID: number}) => {
  return agent.post(`/api/testplans/${testPlanID}/actions/cancel-apitest/${pipelineID}`)
    .then((response: any) => response.body);
};

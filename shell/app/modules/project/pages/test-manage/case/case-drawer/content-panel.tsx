...
import { Edit, Plus, Upload } from '@icon-park/react';
...
const ContentPanel = ({ title, children, className = '', loading = false, append, onClick = noop, mode = 'common' }: IProps) => {
  ...
  const typeIcon = {
    edit: <span onClick={onClick} className="color-text-desc hover-active"><Edit className="mr4" />{i18n.t('project:edit')}</span>,
    add: <span onClick={onClick} className="color-text-desc hover-active"><Plus className="mr4" />{i18n.t('common:add')}</span>,
    upload: <span onClick={onClick} className="color-text-desc hover-active"><Upload className="mr4" />{i18n.t('project:upload')}</span>,
  };
  return (

    ...
    <div className="cluster-log-step-wrap">
      {
        steps.map((step: any) => {
          ...
          return (
            <div key={id} className={`one-step ${curStatus} ${activeStep === id ? 'on-active' : ''}`} onClick={() => !disabled && clickItem(id)}>
...
export const ClusterLog = ({ recordID, onClose }: {
  ...
  const switchLog = <Switch checkedChildren={i18n.t('application:error')} unCheckedChildren={i18n.t('application:standard')} checked={state.isStdErr} onChange={() => updater.isStdErr(!state.isStdErr)} />;

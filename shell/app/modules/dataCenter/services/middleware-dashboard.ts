...
export const getMiddlewares = (payload: MIDDLEWARE_DASHBOARD.IMiddlewaresQuery): MIDDLEWARE_DASHBOARD.IMiddlewaresResp => {
  return agent.get('/api/middlewares')
    .query(payload)
    .then((response: any) => response.body);
};

export const getBaseInfo = (instanceId: string): MIDDLEWARE_DASHBOARD.IBaseInfo => {
  return agent.get(`/api/middlewares/${instanceId}`)
    .then((response: any) => response.body);
};

export const getResourceList = (instanceId: string): MIDDLEWARE_DASHBOARD.IResource[] => {
  return agent.get(`/api/middlewares/${instanceId}/actions/get-resource`)
    .then((response: any) => response.body);
};

export const getAddonUsage = (query?: MIDDLEWARE_DASHBOARD.AddonUsageQuery): MIDDLEWARE_DASHBOARD.AddonUsage => {
  return agent.get('/api/middlewares/resource/classification')
    .query(query)
    .then((response: any) => response.body);
};

export const getAddonDailyUsage = (query?: MIDDLEWARE_DASHBOARD.AddonUsageQuery): MIDDLEWARE_DASHBOARD.AddonDailyUsage => {
  return agent.get('/api/middlewares/resource/daily')
    .query(query)
    .then((response: any) => response.body);
};
...
export const scale = (payload: MIDDLEWARE_DASHBOARD.IScaleData) => {
  return agent.post('/api/addons/actions/scale')
    .send(payload)
    .then((response: any) => response.body);
};

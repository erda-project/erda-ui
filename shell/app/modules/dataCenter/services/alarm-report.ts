...
export const getReportTaskRecord = (id: number): COMMON_ALARM_REPORT.ReportRecord => {
  return agent.get(`/api/report/histories/${id}`)
    .then((response: any) => response.body);
};

export const getSystemDashboards = (payload: Merge<IPagingReq, IScope>): IPagingResp<COMMON_ALARM_REPORT.DashboardBlock> => {
  return agent.get('/api/dashboard/system/blocks')
    .query(payload)
    .then((response: any) => response.body);
};

export const getReportTypes = (): IPagingResp<COMMON_ALARM_REPORT.ReportType> => {
  return agent.get('/api/report/types')
    .then((response: any) => response.body);
};

...
export const getAuthInfo = ({ packageId, apiId }: GATEWAY.GetAuthInfo): GATEWAY.AuthInfoItem[] => {
  return agent.get(`/api/gateway/openapi/packages/${packageId}/apis/${apiId}/authz`)
    .then((response: any) => response.body);
};

export const updateAuthInfo = ({ packageId, apiId, consumers }: GATEWAY.UpdateAuthInfo): boolean => {
  return agent.post(`/api/gateway/openapi/packages/${packageId}/apis/${apiId}/authz`)
    .send({ consumers })
    .then((response: any) => response.body);
};
...
export const getRegisterApps = (params: GATEWAY.GetRegisterApp): { apps: GATEWAY.RegisterApp[] } => {
  return agent.get('/api/gateway/register-apps')
    .query({ ...params })
    .then((response: any) => response.body);
};

export const getApiPackageList = (params: Merge<GATEWAY.Base, Partial<GATEWAY.Query>>): IPagingResp<GATEWAY.ApiPackageItem> => {
  return agent.get('/api/gateway/openapi/packages')
    .query({ ...params })
    .then((response: any) => response.body);
};

export const createApiPackage = ({ orgId, projectId, env, scene, bindDomain, name, authType, aclType, description, needBindCloudapi }: GATEWAY.CreatApiPackege): GATEWAY.ApiPackageItem => {
  return agent.post('/api/gateway/openapi/packages')
    .query({ orgId, projectId, env })
    .send({ name, description, scene, bindDomain, authType, aclType, needBindCloudapi })
    .then((response: any) => response.body);
};

export const updateApiPackage = ({ id, scene, bindDomain, name, authType, aclType, description, needBindCloudapi }: GATEWAY.UpdataApiPackage): GATEWAY.ApiPackageItem => {
  return agent.patch(`/api/gateway/openapi/packages/${id}`)
    .send({ name, description, scene, bindDomain, authType, aclType, needBindCloudapi })
    .then((response: any) => response.body);
};

export const deletePackage = ({ packageId }: {packageId: number}): boolean => {
  return agent.delete(`/api/gateway/openapi/packages/${packageId}`)
    .then((response: any) => response.body);
};

// 调用方授权
export const getConsumerAuthorizes = ({ packageId }: GATEWAY.Package): GATEWAY.AuthInfoItem[] => {
  return agent.get(`/api/gateway/openapi/packages/${packageId}/consumers`)
    .then((response: any) => response.body);
};

export const getApiPackageDetail = ({ id }: { id: string }): GATEWAY.ApiPackageItem => {
  return agent.get(`/api/gateway/openapi/packages/${id}`)
    .then((response: any) => response.body);
};

export const createOpenApiConsumer = ({ description, name, ...restParams }: GATEWAY.CreateOpenApiConsumer): string => {
  return agent.post('/api/gateway/openapi/consumers')
    .query(restParams)
...
export const updateConsumerAuthorizes = ({ packageId, data }: GATEWAY.UpdateConsumerAuthByConsumers): boolean => {
  return agent.post(`/api/gateway/openapi/packages/${packageId}/consumers`)
    .send(data)
    .then((response: any) => response.body);
};

export const getPackageDetailApiList = ({ packageId, ...restParams }: GATEWAY.GetPackageDetailApiList): IPagingResp<GATEWAY.PackageDetailApiListItem> => {
  return agent.get(`/api/gateway/openapi/packages/${packageId}/apis`)
    .query({ ...restParams })
    .then((response: any) => response.body);
};
...
export const importApis = ({ packageId, ...restParams }: {[k: string]: any; packageId: number}) => {
  return agent.post(`/api/gateway/openapi/packages/${packageId}/loadserver`)
    .send({ ...restParams })
    .then((response: any) => response.body);
};

export const createPackageApi = ({ packageId, ...rest }: Merge<GATEWAY.CreatePackageApi, GATEWAY.Package>) => {
  return agent.post(`/api/gateway/openapi/packages/${packageId}/apis`)
    .send(rest)
    .then((response: any) => response.body);
};

export const updatePackageApi = ({ packageId, apiId, ...rest }: Merge<GATEWAY.CreatePackageApi, {packageId: string; apiId: string}>) => {
  return agent.patch(`/api/gateway/openapi/packages/${packageId}/apis/${apiId}`)
    .send(rest)
    .then((response: any) => response.body);
};

export const deletePackageApi = ({ packageId, apiId }: {packageId: string; apiId: string}): boolean => {
  return agent.delete(`/api/gateway/openapi/packages/${packageId}/apis/${apiId}`)
    .then((response: any) => response.body);
};

export const getServiceRuntime = (params: GATEWAY.GetRuntimeDetail): GATEWAY.RuntimeDetail[] => {
  return agent.get('/api/gateway/openapi/service-runtime')
    .query(params)
    .then((response: any) => response.body);
};

export const getApiPackages = (params: GATEWAY.Base): GATEWAY.ApiPackageItem[] => {
  return agent.get('/api/gateway/openapi/packages-name')
    .query({ ...params })
    .then((response: any) => response.body);
};

export const getApiConsumers = (params: GATEWAY.Base): GATEWAY.ConsumersName[] => {
  return agent.get('/api/gateway/openapi/consumers-name')
    .query({ ...params })
    .then((response: any) => response.body);
};

export const getOpenApiConsumerList = (params: Merge<GATEWAY.Base, Partial<GATEWAY.Query>>): IPagingResp<GATEWAY.ConsumersName> => {
  return agent.get('/api/gateway/openapi/consumers')
    .query({ ...params })
    .then((response: any) => response.body);
};
export const getConsumerAuthPackages = ({ consumerId }: { consumerId: string }): GATEWAY.ConsumerAuthPackages[] => {
  return agent.get(`/api/gateway/openapi/consumers/${consumerId}/packages`)
    .then((response: any) => response.body);
};
export const updateConsumerAuthPackages = ({ consumerId, ...rest }: GATEWAY.UpdateConsumerAuthByPackages): boolean => {
  return agent.post(`/api/gateway/openapi/consumers/${consumerId}/packages`)
    .send({ ...rest })
    .then((response: any) => response.body);
};
export const updateOpenApiConsumer = ({ id, description }: { id: string; description: string }): GATEWAY.ConsumersName => {
  return agent.patch(`/api/gateway/openapi/consumers/${id}`)
    .send({ description })
    .then((response: any) => response.body);
};

export const getConsumerDetail = (consumerId: string): GATEWAY.ConsumerDetail => {
  return agent.get(`/api/gateway/consumer/${consumerId}`)
    .then((response: any) => response.body);
};

export const getConsumerCredentials = ({ consumerId }: { consumerId: string }): GATEWAY.ConsumerDetail => {
  return agent.get(`/api/gateway/openapi/consumers/${consumerId}/credentials`)
    .then((response: any) => response.body);
};
export const updateConsumerCredentials = ({ authConfig, consumerId }: GATEWAY.UpdateCredentials): GATEWAY.IAuthConfig => {
  return agent.post(`/api/gateway/openapi/consumers/${consumerId}/credentials`)
    .send({ authConfig })
    .then((response: any) => response.body);
};

export const deleteOpenApiConsumer = ({ consumerId }: { consumerId: string }): boolean => {
  return agent.delete(`/api/gateway/openapi/consumers/${consumerId}`)
    .then((response: any) => response.body);
};

export const getPolicyList = ({ category, ...params }: Merge<GATEWAY.Base, { category: string }>): { category: string; description: string; policyList: GATEWAY.PolicyListItem[] } => {
  return agent.get(`/api/gateway/policies/${category}`)
    .query(params)
    .then((response: any) => response.body);
};

export const getAPIList = (params: Merge<GATEWAY.GetApiList, GATEWAY.ApiFilter>): GATEWAY.ApiResponse => {
  return agent.get('/api/gateway/api')
    .query(params)
    .then((response: any) => response.body);
};

export const addAPI = (data: GATEWAY.AddAPI): { apiId: string } => {
  return agent.post('/api/gateway/api')
    .send(data)
    .then((response: any) => response.body);
};

export const updateAPI = (data: Merge<GATEWAY.AddAPI, { apiId: string }>): GATEWAY.ApiListItem => {
  return agent.patch(`/api/gateway/api/${data.apiId}`)
    .send(data)
    .then((response: any) => response.body);
};

export const deleteAPI = (apiId: string): boolean => {
  return agent.delete(`/api/gateway/api/${apiId}`)
    .then((response: any) => response.body);
};

export const getApiDomain = (params: GATEWAY.GetDomain): GATEWAY.ApiDomain => {
  return agent.get('/api/gateway/domain')
    .query({ ...params })
    .then((response: any) => response.body);
};
...
export const saveApiDomain = (params: GATEWAY.SaveDomain): GATEWAY.ApiDomain => {
  ...
  return agent.put('/api/gateway/domain')
    .query({ ...query })
...
export const createApiLimit = ({ orgId, projectId, env, ...rest }: Merge<GATEWAY.updateLimit, GATEWAY.Base>): boolean => {
  return agent.post('/api/gateway/openapi/limits')
    .query({ orgId, projectId, env })
    .send(rest)
    .then((response: any) => response.body);
};
...
export const saveSafetyWaf = (params: GATEWAY.SaveWrf): GATEWAY.SafetyWaf => {
  ...
  return agent.put('/api/gateway/policies/safety-waf')
    .query({ ...query })
...
export const saveSafetyIP = (params: GATEWAY.SaveIp): GATEWAY.SafetyIp => {
  ...
  return agent.put('/api/gateway/policies/safety-ip')
    .query({ ...query })
...
export const saveSafetyServerGuard = (params: GATEWAY.SaveServerGuard): GATEWAY.SafetyServerGuard => {
  ...
  return agent.put('/api/gateway/policies/safety-server-guard')
    .query({ ...query })
...
export const saveSafetyCsrf = (params: GATEWAY.SaveCsrf): GATEWAY.SafetyCsrf => {
  const { global, csrfEnable, userCookie, excludedMethod, tokenName, tokenDomain, cookieSecure, validTTL, refreshTTL, errStatus, errMsg, ...query } = params;
  return agent.put('/api/gateway/policies/safety-csrf')
    .query({ ...query })
    .send({ global, switch: csrfEnable, userCookie, excludedMethod, tokenName, tokenDomain, cookieSecure, validTTL, refreshTTL, errStatus, errMsg })
    .then((response: any) => response.body);
};
...
export const saveBusinessCors = (params: GATEWAY.SaveCors): GATEWAY.BusinessCors => {
  const { packageId, apiId, ...body } = params;
  return agent.put('/api/gateway/policies/cors')
    .query({ packageId, apiId })
    .send(body)
    .then((response: any) => response.body);
};
...
export const saveBusinessCustom = (params: GATEWAY.SaveCustom): GATEWAY.BusinessCustom => {
  const { packageId, apiId, ...body } = params;
  return agent.put('/api/gateway/policies/custom')
    .query({ packageId, apiId })
    .send(body)
    .then((response: any) => response.body);
};
...
export const getServiceApiPrefix = (payload: GATEWAY.QueryApiPrefix): string[] => {
  return agent.get('/api/gateway/openapi/service-api-prefix')
    .query(payload)
    .then((response: any) => response.body);
};

export const getCloudApiInfo = (packageId: Omit<GATEWAY.Base, 'orgId'>): GATEWAY.CloudapiExists => {
  return agent.get('/api/gateway/openapi/cloudapi-info')
    .query(packageId)
    .then((response: any) => response.body);
};

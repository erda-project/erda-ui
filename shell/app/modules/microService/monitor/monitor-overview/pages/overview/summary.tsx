...
const Summary = () => {
  ...
  const aiCapacityConf = {
    webAvg: {
      name: i18n.t('microService:response time'),
      value: getFormatter('TIME').format(aiCapacityData.webAvg),
      color: 'primary',
    },
    webCpm: {
      name: i18n.t('microService:throughput'),
  ...
  const pageCapacityConf = {
    apdex: {
      name: `Apdex ${i18n.t('microService:index')}`,
      value: `${floor(biCapacityData.apdex, 2)} T`,
      color: 'primary',
    },
    ajaxResp: {
      name: `Ajax ${i18n.t('microService:response time')}`,
      value: getFormatter('TIME').format(biCapacityData.ajaxResp),
      color: 'primary',
    },
    ajaxCpm: {
      name: `Ajax ${i18n.t('microService:throughput')}`,
      value: `${floor(biCapacityData.ajaxCpm, 2)} cpm`,
      color: 'info',
    },
    ajaxErr: {
      name: `Ajax ${i18n.t('microService:error rate')}`,
    ...
    <Row gutter={24}>
      <Col span={12}>
        <GridPanel
          ...
          title={i18n.t('microService:page performance indicator (within 1 hour)')}
      ...
      <Col span={12}>
        <GridPanel
          className="little-panel"
          title={i18n.t('microService:application performance indicators (within 1 hour)')}

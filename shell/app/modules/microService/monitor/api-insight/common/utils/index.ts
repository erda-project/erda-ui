...
export const chartMapCreator = (name: string, payload?: object) => {
  const commonMap = {
    performanceInterval: {
      fetchApi: 'ta_performance/histogram',
      query: { avg: ['tcp', 'srt', 'plt', 'rlt'] },
      // dataHandler: handlerName,
  ...
  return { ...commonMap[name], ...payload } || null;
};

export const sortCreator = (name: string, payload?: object) => {
  const commonSort = {
    sortTab: {
      type: 'sortTab',
      tabList: [
        { name: i18n.t('microService:average time'), key: 'time' },
        { name: i18n.t('microService:time percent'), key: 'percent' },
  ...
  return { ...commonSort[name], ...payload } || null;
};

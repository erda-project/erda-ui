...
export const ApiMap = {
  sortList: {
    getFetchObj: ({ sortTab }: {sortTab: string}) => {
      const fetchMap = {
        time: { fetchApi: 'ta_m_top_avg_time', query: { group: 'device', limit: 20, sort: 'avg_plt', avg: 'plt' }, dataKey: 'avg.plt' },
        percent: { fetchApi: 'ta_m_top_percent_time', query: { group: 'device', limit: 20, sort: 'sumPercent_plt', sumPercent: 'plt' }, dataKey: 'sumPercent.plt' },
        cpm: { fetchApi: 'ta_m_top_cpm', query: { group: 'device', limit: 20, sort: 'cpm_plt', cpm: 'plt' }, dataKey: 'cpm.plt' },
      };

...
export const ApiMap = {
  sortList: {
    getFetchObj: ({ sortTab }: {sortTab: string}) => {
      const fetchMap = {
        time: { fetchApi: 'ta_m_top_avg_time', query: { group: 'av', avg: 'plt', limit: 20, sort: 'avg_plt' }, dataKey: 'avg.plt' },
        percent: { fetchApi: 'ta_m_top_percent_time', query: { group: 'av', sumPercent: 'plt', limit: 20, sort: 'sumPercent_plt' }, dataKey: 'sumPercent.plt' },
        cpm: { fetchApi: 'ta_m_top_cpm', query: { group: 'av', cpm: 'plt', limit: 20, sort: 'cpm_plt' }, dataKey: 'cpm.plt' },
      };

...
import { isArray, reduce, isEmpty, find, uniq, map, filter, merge, groupBy, maxBy, values, keys, get, min, flatten, minBy, sortBy, cloneDeep, set } from 'lodash';
// @ts-ignore
...
export const renderTopology = (_originData: TOPOLOGY.ITopologyResp, snap: any, g: any, external: Obj = {}) => {
  let containerWidth = 0;
  let containerHeight = 0;
  const originData = get(_originData, 'nodes');
  if (isEmpty(originData)) return { containerWidth, containerHeight };
  const nodeGroup = dataHandler.getGroupChart(originData);
...
const dataHandler = {
  ...
  getNodesPosition: (nodeMap: object) => {
    let boxWidth = 0;
    let boxHeight = 0;
    const { NODE: { width, height, margin }, direction, padding } = chartConfig;

...
export const getNotifyGroup = (id) => {
  return agent.get(`/api/notify-groups/${id}`).then((response) => response.body);
};

export const updateNotifyGroup = (data) => {
  return agent
    .put(`/api/notify-groups/${data.id}`)
    .send(data)
    .then((response) => response.body);
};

export const deleteNotifyGroup = (data) => {
  return agent.delete(`/api/notify-groups/${data.id}`).then((response) => response.body);
};

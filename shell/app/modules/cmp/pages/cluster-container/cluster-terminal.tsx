...
export const K8sPodTerminalLog = (
  ...
  const handleDownload = (formData: { start: Moment; duration: number; type: string }) => {
    ...
    const logFile = `/api/orgCenter/logs/actions/download?clusterName=${clusterName}&end=${end}&id=${containerId}&source=container&start=${_start}&live=true&containerName=${containerName}&podName=${podName}&podNamespace=${namespace}&isFallBack=true`;

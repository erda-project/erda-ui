...
const InterfaceForm = ({ visible, editMode, az, interfacename, onCancel }: IProps) => {
  ...
  const fieldsList = [
    {
      label: i18n.t('msp:interface name'),
      getComp: () => interfacename,
    },
    {
      label: i18n.t('msp:load balancing'),
      name: 'lb_type',
      type: 'select',
    ...
    {
      label: i18n.t('msp:number of requests processed per connection'),
    ...
    {
      label: i18n.t('msp:maximum number of requests'),
      name: 'max_requests',
      type: 'select',
  ...
  return (
    <FormModal
      title={i18n.t('msp:interface configuration')}
      visible={visible}

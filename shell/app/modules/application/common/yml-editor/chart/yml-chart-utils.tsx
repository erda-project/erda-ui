...
import { externalKey, CHART_CONFIG, NodeType, NodeEleMap } from '../config';
...
const getNodePostion = (data: IData[][], chartConfig: IChartConfig, external: IExternal) => {
  ...
  const maxWidthBox = maxBy(boxSizeArr, item => item.width) as Obj;
...
const renderNodes = (nodeData: any[][], chart: any, chartConfig: IChartConfig, external: any, chartWidth: number) => {
  ...
  map(nodeData, (nodeList, index) => {
    ...
    if (isEdit && curStartNode.nodeType !== NodeType.startNode && curStartNode.nodeType !== NodeType.addRow) {
      ...
      listG.mouseover(() => {
        if (isEdit) {
          const addEle = document.getElementById(`_yml-node-add-${index}_`);
          if (addEle) {
    ...
    map(nodeList, (node, subIndex) => {
      ...
      const fobjectSVG = `<foreignObject id="${nodeId}" class="svg-model-node-carrier" x="${xPos}" y="${yPos}" width="${width}" height="${height}"></foreignObject>`;
      // g标签上加id，用于设置opcity属性（兼容safari）
      const g = chart.g().attr({ id: `${nodeId}-g` });
      ...
      if (isEdit && nodeLen === subIndex + 1 && curStartNode.nodeType !== NodeType.startNode && curStartNode.nodeType !== NodeType.addRow) { // 编辑态，末尾追加添加节点
        const add_xPos = x + width / 2 + MARGIN.X;
        const add_yPos = y - height / 2;
        const addNodeId = `_yml-node-add-${index}_`;
        ...
        // g标签上加id，用于设置opcity属性（兼容safari）
        const addG = chart.g().attr({ id: `${nodeId}-g` });
        ...
        if (targetNodes) {
          ...
          map(targetNodes, targetNode => {
            ...
            linkData.push({
              sourceNode: subItem,
              targetNode,
              id: `${startPos.nodeId}_${endPos.nodeId}`,

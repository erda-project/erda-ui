...
const PurePipelineNodeForm = (props: IEditStageProps & FormComponentProps) => {
  ...
  const renderStructArray = (property: any, parentKey: string) => {
    ...
    const addBtn = editing ?
      <Plus className="pointer" onClick={() => addNewItemToStructArray(property, property.struct[0])} /> : null;
...
const PipelineNodeForm = Form.create()(PurePipelineNodeForm);

interface IProps extends Omit<IEditStageProps, 'chosenActionName' | 'chosenAction'>{
  closeDrawer: () => void;
  visible: boolean;
}

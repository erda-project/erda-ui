...
export const APP_TABS = () => {
  ...
  const [appDetail, initAppInfo] = appStore.useStore((s) => [s.detail, s.initAppInfo]);
  const repoInfo = repoStore.useStore((s) => s.info);
  const [currentRoute, routes] = routeInfoStore.useStore((s) => [s.currentRoute, s.routes]);

  const lastRouteWithPath = find(routes, 'relativePath');
  const lastPathPart = last(location.pathname.split('/')) || '';
  const curActiveKey =
    currentRoute.mark !== 'application'
      ? lastRouteWithPath?.alwaysShowTabKey || lastRouteWithPath?.tabKey || lastPathPart
      : '';

  const [curKey, setCurKey] = React.useState(curActiveKey);

  React.useEffect(() => {
    currentRoute.mark !== 'application' && setCurKey(curActiveKey);
  }, [curActiveKey, currentRoute.mark]);
  ...
  const tabs = React.useMemo(() => {
    ...
    return filter(modeMap[mode], (item: ITab) => item.show !== false) as ROUTE_TABS[];
  }, [perm, mode, isExternalRepo, repoInfo, commitKey, branchKey, mrKey]);

  React.useEffect(() => {
    if (currentRoute.mark === 'application' && initAppInfo) {
      const useTabs = tabs.filter((item) => item.show !== false && item.disabled !== true);
      const curTab =
...
export const AppPageWrapper = (Comp: () => JSX.Element) => {
  return () => {
    const appId = routeInfoStore.useStore((s) => s.params.appId);
    return <Comp key={appId} />;
  };
};

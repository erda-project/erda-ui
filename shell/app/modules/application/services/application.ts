...
export const initApp = ({ applicationID, ...rest }: APPLICATION.initApp): string => {
  return agent.put(`/api/applications/${applicationID}/actions/init`)
    .send(rest)
    .then((response: any) => response.body);
};

export const queryTemplate = (payload: APPLICATION.queryTemplate) => {
  return agent.get('/api/applications/actions/list-templates')
    .query(payload)
    .then((response: any) => response.body);
};
...
export const getBranchInfo = ({ appId }: { appId: number }): APPLICATION.IBranchInfo[] => {
  return agent.get('/api/cicds/actions/app-all-valid-branch-workspaces')
    .query({ appID: appId })
    .then((response: any) => response.body);
};

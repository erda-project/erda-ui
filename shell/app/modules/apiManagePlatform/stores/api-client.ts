...
import * as Service from '../services/api-client';
...
const apiClient = createStore({
  ...
  effects: {
    async getClientList({ call, update }, payload: API_CLIENT.QueryClient) {
      const { list } = await call(Service.getClientList, payload, { paging: { key: 'clientListPaging' } });
      update({ clientList: list });
      return list;
    },
    async createClient({ call }, payload: API_CLIENT.CreateClient) {
      const res = await call(Service.createClient, payload);
      return res;
    },
    ...
    async getClientDetail({ call, update }, payload: API_CLIENT.Common) {
      const clientDetail = await call(Service.getClientDetail, payload);
    ...
    async createContract({ call }, payload: API_CLIENT.CreteContract) {
      const res = await call(Service.createContract, payload);
      return res;
    },
    async getContractList({ call, update }, payload: API_CLIENT.QueryContractList) {
      ...
      const res = await call(Service.getContractList, payload, { paging: { key: pagingKey } });
      update({ [listKey]: res.list || [] });
      return res;
    },
    async getContractDetail({ call, update }, payload: API_CLIENT.QueryContract) {
      const contractDetail = await call(Service.getContractDetail, payload);

...
const apis = {
  getServiceTypes: {
    ...
    mock() {
      return [
        ...
        { displayName: 'Addon', name: 'addon' },
        { displayName: 'App', name: 'app' },
      ];
    },
  },
  getServiceList: {
    ...
    mock(payload) {
      ...
      const number = numberMap[`${payload.type}${payload.pageNo}`];
      return {
  ...
  getServiceDetail: {
    ...
    mock() {
      return {
        list: new Array(3).fill('').map((item, idx) => ({
          id: `${idx}`,
          orgName: 'Erda',
          type: 'Action',
          name: 'detailName',
          version: ['1.0', '1.1', '1.2'][idx],
          displayName: 'detailName',
          summary: '测试测试测试测试测试测试测试测试测试测试测试测试',
          labels: ['aaa', 'bbbbbbbbbbbbb', 'cccc', 'dd', 'aaa2', 'bbbbbbbbbbbbb2', 'cccc2', 'dd2'],
          logoURL:
            'https://terminus-paas.oss-cn-hangzhou.aliyuncs.com/paas-doc/2020/02/14/58faae51-f602-45c3-95e9-d2b4fe4a369c.png',
          presentation: {
            desc: 'git-checkout action 提供对 Git 代码仓库的克隆能力，包括设置 depth、submodules 等功能。',
...
export const getServiceTypes = apiCreator<() => MARKET.Type[]>(apis.getServiceTypes);

export const getServiceList = apiCreator<(p: MARKET.ServiceReq) => IPagingResp<MARKET.Service>>(apis.getServiceList);

export const getServiceDetail = apiCreator<(p: { id: string }) => IPagingResp<MARKET.Service>>(apis.getServiceDetail);

...
export const getVersionList = (query: PUBLISHER.VersionListQuery): IPagingResp<PUBLISHER.IVersion> => {
  ...
  return agent.get(`/api/publish-items/${artifactsId}/versions`)
    .query(rest)
    .then((response: any) => response.body);
};
...
export const setGrayAndPublish = (query: PUBLISHER.IUpdateGrayQuery) => {
  ...
  return agent.post(`/api/publish-items/versions/actions/${action}`)
    .send(rest)
    .then((response: any) => response.body);
};
...
export const getOnlineVersionData = (query: PUBLISHER.IOnlineVersionQuery): IPagingResp<PUBLISHER.IOnlineVersion> => {
  ...
  return agent.get(`/api/publish-items/${publishItemId}/versions/actions/public-version`)
...
export const addVersion = (data: PUBLISHER.IVersion) => {
  ...
  return agent.post(`/api/publish-items/${artifactsId}/versions`)
    .send(rest)
    .then((response: any) => response.body);
};
...
export const getArtifactDetail = (id: number | string): PUBLISHER.IArtifacts => {
  return agent.get(`/api/publish-items/${id}`)
    .then((response: any) => response.body);
};

export const getLibVersion = (palyload: PUBLISHER.QueryLibVersion): PUBLISHER.LibVersion[] => {
  return agent.get('/api/lib-references/actions/fetch-versions')
    .query(palyload)
    .then((response: any) => response.body);
};

export const getH5PackageNames = ({ publishItemId }: { publishItemId: string }): string[] => {
  return agent.get(`/api/publish-items/${publishItemId}/versions/actions/get-h5-packagename`)
    .then((response: any) => response.body);
};


export const getAKAIByPublishItemId = ({ publishItemId }: { publishItemId: string }): Record<string, PUBLISHER.MonitorItem> => {
  return agent.get(`/api/publish-items/${publishItemId}/list-monitor-keys`)
    .then((response: any) => response.body);
};

export const uploadOfflinePackage = ({ publishItemId, data }: PUBLISHER.OfflinePackage): PUBLISHER.OfflinePackageType => {
  return agent.post(`/api/publish-items/${publishItemId}/versions/create-offline-version`)
    .send(data)
    .then((response: any) => response.body);
};

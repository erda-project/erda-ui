...
const FilterVersion = ({ value, onChange, className, groups }: { value?: string; onChange: (val: string | undefined) => void; className?: string; groups: Array<{ label: string; value: string }> }) => {
  return (
    <Select
      className={`${className || ''}`}
      onChange={(val: SelectValue) => { onChange(val as string); }}
      allowClear
      value={value}
...
const ErrorList = ({ artifactsId, timeSpan, version, monitorKey }: { artifactsId: string; timeSpan: ITimeSpan; version: string | undefined; monitorKey: PUBLISHER.MonitorKey}) => {
  ...
  React.useEffect(() => {
    getErrorList({ artifactsId, start: timeSpan.startTimeMs, end: timeSpan.endTimeMs, filter_av: version, ...monitorKey });
  ...
  return (
    <div className="artifacts-error-report">
      ...
      <div className="mt16">
        ...
        <FilterVersion className="version-selector" value={lineVersion} onChange={updater.lineVersion} groups={groups} />
      </div>

  ...
  spec.forEach(specItem => {
    ...
    if(!name.startsWith('_')){
      ...
      props.forEach(prop=>{
        ...
        }else if(attrType.includes('|')){ // a | b | c 或者 string | number | boolean
        ...
        }else if(attrType.endsWith('[]')){ // 数组类型
...
const dealTypeSpec = (spec)=>{
  ...
  spec.forEach(specItem=>{
    ...
    if(name!=='Props'){ // 过滤MakeProps<Spec>
      if(value.startsWith('{')){// 当Obj处理
        codeArr.push(`type ${name} map[string]interface{}\n`);
      }else if(value.includes('|')){ // 当枚举处理
